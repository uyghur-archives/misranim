...
function Ajax(recvType, waitId) {
	...
	aj.loading = 'ئېچىلىۋاتىدۇ. سەل ساقلاڭ ...';
...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : 'سەل ساقلاڭ ...';
...
function showMenu(v) {
	...
	if(!menuObj.getAttribute('initialized')) {
		...
		if(cover) {
			...
			coverObj.style.right = coverObj.style.top = '0px';
...
function setMenuPosition(showid, menuid, pos) {
	...
	switch(direction) {
		...
		case 4:
			ml = bpl;
			if(ml && BROWSER.ie && BROWSER.ie < 8){ 
				ml = ml - mw - parseInt(sw/2); 
				if(menuid == 'qmenu_menu') ml = sx;
			}
			mt = bpt;
			break;
		case 3:
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		leftmsg = locationtime + ' سېكونتتىن كېيىن بەت ئالمىشىدۇ';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : 'قالدۇرۇش';
	...
	s += t ? t : 'ئەسكەرتىش ئۇچۇرى';
	s += '</em><span><a href="javascript:;" id="fwin_dialog_close" class="flbc" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="تاقاش">تاقاش</a></span></h3>';
	if(mode == 'info') {
		s += msg ? msg : '';
	} else {
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> تەخىرقىلىڭ...')}, 500);
		}
	};
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog(msg, 'alert', 'خاتالىق ئۇچۇرى', null, true, null, '', '', '', 3);
	}
}
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == 'ئىزدەيدىغان مەزمۇن') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = 'ئىزدەيدىغان مەزمۇن';
	} else {
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
				searchtxt.value = 'ئىزدەيدىغان مەزمۇن';
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
function searchFocus(obj) {
	if(obj.value == 'ئىزدەيدىغان مەزمۇن') {
		obj.value = '';
	}
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '【　　　】' : '【يېڭى ئەسكەرتمە】') + NOTICETITLE['oldTitle'];
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("Ctrl+D كونۇپكىسىنى بېسىپ ساقلىغۇچقا ساقلاڭ", 'notice');
		}
	}
}
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog("IEدىن باشقا تور كۆرگۈچكە قولدا باش بەت قىلىۋېلىڭ!", 'notice');
		doane();
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'زىيارەت ياردەمچىسىنى ئېچىش';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'زىيارەت ياردەمچىسىنى تاقاش';
		}
	}
}
