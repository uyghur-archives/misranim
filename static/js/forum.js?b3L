...
function keyPageScroll(e, prev, next, url, page) {
	...
	if(next && actualCode == 37) {
		window.location = url + '&page=' + (page + 1);
	}
	if(prev && actualCode == 39) {
		window.location = url + '&page=' + (page - 1);
	}
}
...
function removeindexheats() {
	return confirm('بۇ تېمىنى قىزغىن تېمىلار قاتارىدىن چىقىرىۋىتەمسىز؟');
}
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(theform.message.value == '' || theform.subject.value == '') {
		s = 'كەچۈرۈڭ، ماۋزۇ ياكى مەزمۇن كىرگۈزمىدىڭىز';
		theform.message.focus();
	} else if(mb_strlen(theform.subject.value) > 180) {
		s = 'ماۋزۇ 80 ھەرپتىن ئېشىپ كەتتى';
		theform.subject.focus();
	}
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = 'يازمىڭىزنىڭ ئۇزۇنلىقى ئۆلچەمگە چۈشمەيدۇ。\n\nھازىرقى ئۇزۇنلۇقى: ' + mb_strlen(theform.message.value) + ' ' + 'ھەپ\nسىستېما چېكى: ' + postminchars + ' ھەرپتىن ' + postmaxchars + ' ھەرپكىچە';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('ئەسلىگە كەلتۈرگىلى بولىدىغان مەزمۇن يوق', 'info');
		}
		return;
	}

	if(!quiet && !confirm('بۇ مەشغۇلات ھازىرقى يازمىنى باستۇرىۋىتىدۇ؟ ئەسلىگە قايتۇرامسىز؟')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">يېڭى ئىنكاسلىق تېما بار، بۇ يەرنى چېكىپ كۆرۈڭ', 'colspan': colspan }}};
...
function showtime() {
	for(i=0; i<=DTimers.length; i++) {
		if(DItemIDs[i]) {
			if(DTimers[i] == 0) {
				$(DItemIDs[i]).innerHTML = 'ئاخىرلاشتى';
				DItemIDs[i] = '';
				continue;
			}
			var timestr = '';
			...
			if(timer_day > 0) {
				timestr += timer_day + 'كۈن';
			}
			if(timer_hour > 0) {
				timestr += timer_hour + 'سائەت'
			}
			if(timer_minute > 0) {
				timestr += timer_minute + 'مىنۇت '
			}
			if(timer_second > 0) {
				timestr += timer_second + 'سېكۇنت '
			}
