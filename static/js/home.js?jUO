		...
		theImages[i].onload = function() {
			if (this.width > size) {
				...
				if (this.parentNode.tagName.toLowerCase() != 'a') {
					...
					this.title = 'رەسىمنى چېكىپ، يېڭى كۆزنەكتە ئەسلى رازمېرىنى كۆرۈڭ';
...
function ischeck(id, prefix) {
	...
	for(var i = 0; i < form.elements.length; i++) {
		...
		if(e.name.match(prefix) && e.checked) {
			if(confirm("راستىنلا بۇ مەشغۇلاتنى قىلامسىز")) {
				return true;
			} else {
				return false;
			}
	...
	alert('مەشغۇلات ئوبىكتىنى تاللاڭ');
	return false;
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	var videoMp3 = true;
	if('' == flashvar) {
		alert('رەسىم ئادېرسى خاتا، قۇرۇق قويماڭ');
		return false;
	}
	if('music' == host) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode("يېغىش");
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>سىلەر دوست بولدۇڭلار، ئەمدى：<a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">ئۇنىڭغا خەت قالدۇرۇڭ</a> ，ياكى <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">سالاملىشىڭ</a></p>';
	}
	showCreditPrompt();
}
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; جەدۋەلنى يېغىش';
	$(opid).onclick = function() {
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; تېخىمۇ كۆپ يېڭىلىق';
	$(opid).onclick = function() {
